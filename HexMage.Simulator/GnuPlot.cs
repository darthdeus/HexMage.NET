...
namespace HexMage.Simulator {
    public class GnuPlot {
            ...
            for (int i = 0; i < storedPlots.Count; i++) {
                ...
                switch (p.PlotType) {
                    ...
                    case PlotTypes.ContourFileOrFunction:
                        contfile = System.IO.Path.GetTempPath() + "_cntrtempdata" + i + ".dat";
        ...
        public static void SaveSetState(string filename = null) {
            if (filename == null)
                filename = System.IO.Path.GetTempPath() + "setstate.tmp";
        ...
        static void makeContourFile(string fileOrFunction, string outputFile)
        ...
        static void setContourLabels(string contourFile) {
            ...
            while ((line = file.ReadLine()) != null) {
                if (line.Contains("label:")) {
                    ...
                    GnupStWr.WriteLine("set object " + ++contourLabelCount + " rectangle center " + c[0] + "," + c[1] +
                                       " size char " + (c[2].ToString().Length + 1) +
                                       ",char 1 fs transparent solid .7 noborder fc rgb \"white\"  front");
                    GnupStWr.WriteLine("set label " + contourLabelCount + " \"" + c[2] + "\" at " + c[0] + "," + c[1] +
                                       " front center");
                }
            }
            file.Close();
